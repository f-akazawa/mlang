...
#ifndef MLANG_SOURCELOCATION_H
#define MLANG_SOURCELOCATION_H

#include "mlang/Basic/LLVM.h"
...
namespace llvm {
  ...
  /// DenseSets.
  template <>
  struct DenseMapInfo<mlang::FileID> {
    static inline mlang::FileID getEmptyKey() {
      return mlang::FileID();
    }
    static inline mlang::FileID getTombstoneKey() {
      return mlang::FileID::getSentinel();
    }

    static unsigned getHashValue(mlang::FileID S) {
      return S.getHashValue();
    }

    static bool isEqual(mlang::FileID LHS, mlang::FileID RHS) {
      return LHS == RHS;
    }
  };

  template <>
  struct isPodLike<mlang::SourceLocation> { static const bool value = true; };
  template <>
  struct isPodLike<mlang::FileID> { static const bool value = true; };

  // Teach SmallPtrSet how to handle SourceLocation.
  template<>
  class PointerLikeTypeTraits<mlang::SourceLocation> {
  public:
    static inline void *getAsVoidPointer(mlang::SourceLocation L) {
      return L.getPtrEncoding();
    }
    static inline mlang::SourceLocation getFromVoidPointer(void *P) {
      return mlang::SourceLocation::getFromRawEncoding((unsigned)(uintptr_t)P);
    }
