...
#ifndef MLANG_BASIC_SOURCE_MANAGER_H_
#define MLANG_BASIC_SOURCE_MANAGER_H_
...
public:
	...
	const llvm::MemoryBuffer *getBuffer(Diagnostic &Diag,
			const SourceManager &SM, SourceLocation Loc = SourceLocation(),
...
public:
	...
	FileID createFileID(const FileEntry *SourceFile, SourceLocation IncludePos,
			SrcMgr::CharacteristicKind FileCharacter, unsigned PreallocatedID =
					0, unsigned Offset = 0) {
		const SrcMgr::ContentCache *IR = getOrCreateContentCache(SourceFile);
		assert(IR && "getOrCreateContentCache() cannot return NULL");
		return createFileID(IR, IncludePos, FileCharacter, PreallocatedID,
				Offset);
	}
	...
	FileID createFileIDForMemBuffer(const llvm::MemoryBuffer *Buffer,
			unsigned PreallocatedID = 0, unsigned Offset = 0) {
		return createFileID(createMemBufferContentCache(Buffer),
				SourceLocation(), SrcMgr::C_User, PreallocatedID, Offset);
	}
	...
	SourceLocation createInstantiationLoc(SourceLocation Loc,
			...
			SourceLocation InstantiationLocEnd, unsigned TokLength,
			unsigned PreallocatedID = 0, unsigned Offset = 0);
	...
	void overrideFileContents(const FileEntry *SourceFile,
			const llvm::MemoryBuffer *Buffer, bool DoNotFree = false);
	...
	const FileEntry *getFileEntryForSLocEntry(
	...
	std::pair<FileID, unsigned> getDecomposedLoc(SourceLocation Loc) const {
		...
		return std::make_pair(FID, Loc.getOffset()
				- getSLocEntry(FID).getOffset());
	}
	...
	std::pair<FileID, unsigned> getDecomposedInstantiationLoc(
	...
	unsigned
	getColumnNumber(FileID FID, unsigned FilePos, bool *Invalid = 0) const;
	unsigned
	getSpellingColumnNumber(SourceLocation Loc, bool *Invalid = 0) const;
	unsigned
	getInstantiationColumnNumber(SourceLocation Loc, bool *Invalid = 0) const;
	...
	unsigned
	getLineNumber(FileID FID, unsigned FilePos, bool *Invalid = 0) const;

	unsigned
	getInstantiationLineNumber(SourceLocation Loc, bool *Invalid = 0) const;
	...
	///
	unsigned getLineTableFilenameID(llvm::StringRef Name);
	...
	void AddLineNote(SourceLocation Loc, unsigned LineNo, int FilenameID,
			bool IsFileEntry, bool IsFileExit, bool IsSystemHeader,
	...
	SourceLocation getLocation(const FileEntry *SourceFile, unsigned Line,
			unsigned Col);
	...
	typedef llvm::DenseMap<const FileEntry*, SrcMgr::ContentCache*>::const_iterator
	fileinfo_iterator;
	...
	unsigned sloc_entry_size() const {
		return SLocEntryTable.size();
	}
	...
	unsigned sloc_loaded_entry_size() const {
		return SLocEntryLoaded.size();
	}
	...
	const SrcMgr::SLocEntry &getSLocEntry(unsigned ID, bool *Invalid = 0) const {
		...
		// yet, do so now.
		if (ExternalSLocEntries && ID < SLocEntryLoaded.size()
				&& !SLocEntryLoaded[ID] && ExternalSLocEntries->ReadSLocEntry(
				ID) && Invalid)
			*Invalid = true;
	...
	void PreallocateSLocEntries(ExternalSLocEntrySource *Source,
			unsigned NumSLocEntries, unsigned NextOffset);
...
#endif /*MLANG_BASIC_SOURCE_MANAGER_H_*/
