//===--- Diagnostic.h - Diagnostic Handling for Mlang  ----------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
//  This file defines he Diagnostic-related interfaces.
//
//===----------------------------------------------------------------------===//

#ifndef MLANG_DIAG_DIAGNOSTIC_H_
#define MLANG_DIAG_DIAGNOSTIC_H_

#include "mlang/Diag/DiagnosticIDs.h"
...
public:
	...
	static FixItHint CreateInsertion(SourceLocation InsertionLoc,
			llvm::StringRef Code) {
		FixItHint Hint;
		Hint.RemoveRange = CharSourceRange(SourceRange(InsertionLoc,
				InsertionLoc), false);
...
private:
	...
	void PushDiagStatePoint(DiagState *State, SourceLocation L) {
		...
		DiagStatePoints.push_back(DiagStatePoint(State, FullSourceLoc(Loc,
				*SourceMgr)));
	}

	/// \brief Finds the DiagStatePoint that contains the diagnostic state of
	/// the given source location.
	DiagStatePointsTy::iterator
			GetDiagStatePointForLoc(SourceLocation Loc) const;
	...
	typedef void (*ArgToStringFnTy)(ArgumentKind Kind, intptr_t Val,
			const char *Modifier, unsigned ModifierLen, const char *Argument,
			unsigned ArgumentLen, const ArgumentValue *PrevArgs,
...
public:
	explicit Diagnostic(const llvm::IntrusiveRefCntPtr<DiagnosticIDs> &Diags,
			DiagnosticClient *client = 0, bool ShouldOwnClient = true);
	~Diagnostic();
	...
	void ConvertArgToString(ArgumentKind Kind, intptr_t Val,
			const char *Modifier, unsigned ModLen, const char *Argument,
			unsigned ArgLen, const ArgumentValue *PrevArgs,
			unsigned NumPrevArgs, llvm::SmallVectorImpl<char> &Output) const {
		ArgToStringFn(Kind, Val, Modifier, ModLen, Argument, ArgLen, PrevArgs,
				NumPrevArgs, Output, ArgToStringCookie);
	}
...
public:
	...
	void AddSourceRange(const CharSourceRange &R) const {
		assert(NumRanges < sizeof(DiagObj->DiagRanges)
				/ sizeof(DiagObj->DiagRanges[0])
				&& "Too many arguments to diagnostic!");
		if (DiagObj)
...
inline const DiagnosticBuilder &operator<<(const DiagnosticBuilder &DB,
		const char *Str) {
	DB.AddTaggedVal(reinterpret_cast<intptr_t> (Str), Diagnostic::ak_c_string);
	return DB;
}
...
// Adds a DefnContext to the diagnostic. The enable_if template magic is here
// so that we only match those arguments that are (statically) DefnContexts;
// other arguments that derive from DefnContext (e.g., RecordDecls) will not
// match.
template<typename T>
inline typename llvm::enable_if<llvm::is_same<T, DefnContext>,
		const DiagnosticBuilder &>::type operator<<(
		const DiagnosticBuilder &DB, T *DC) {
	DB.AddTaggedVal(reinterpret_cast<intptr_t> (DC), Diagnostic::ak_declcontext);
	return DB;
}
...
inline DiagnosticBuilder Diagnostic::Report(SourceLocation Loc, unsigned DiagID) {
...
public:
	explicit DiagnosticInfo(const Diagnostic *DO) :
		DiagObj(DO) {
	}
	...
	const CharSourceRange &getRange(unsigned Idx) const {
		assert(Idx < DiagObj->NumDiagRanges
	...
	const FixItHint *getFixItHints() const {
		return DiagObj->NumFixItHints ? &DiagObj->FixItHints[0] : 0;
	}
...
public:
	...
	Diagnostic::Level getLevel() const {
		return Level;
	}
...
#endif /* MLANG_DIAG_DIAGNOSTIC_H_ */
