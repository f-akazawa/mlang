...
//
//===----------------------------------------------------------------------===//
#ifndef MLANG_FRONTEND_PCHBITCODES_H_
#define MLANG_FRONTEND_PCHBITCODES_H_
...
namespace mlang {
  namespace serialization {
    ...
    typedef uint32_t DefnID;

    /// \brief a Decl::Kind/DefnID pair.
    typedef std::pair<uint32_t, DefnID> KindDefnIDPair;
    ...
    public:
      ...
      TypeID asTypeID(unsigned FastQuals) const {
        return (Idx << TypeInfo::FastWidth) | FastQuals;
      }
      static TypeIdx fromTypeID(TypeID ID) {
        return TypeIdx(ID >> TypeInfo::FastWidth);
      }
    };
    ...
    struct UnsafeQualTypeDenseMapInfo {
      static inline bool isEqual(Type A, Type B) { return A == B; }
      static inline Type getEmptyKey() {
        return Type::getFromOpaquePtr((void*) 1);
      }
      static inline Type getTombstoneKey() {
        return Type::getFromOpaquePtr((void*) 2);
      }
      static inline unsigned getHashValue(Type T) {
    ...
    typedef llvm::DenseMap<Type, TypeIdx, UnsafeQualTypeDenseMapInfo>
        TypeIdxMap;
    ...
    enum BlockIDs {
      ...
      /// types and defns used within the AST file.
      DEFNTYPES_BLOCK_ID,
    ...
    enum ASTRecordTypes {
      ...
      /// the Type class. The upper bits, after being shifted and
  ...
  /// \brief Record codes for each kind of declaration.
    ...
    enum StmtCode {
      ...
      STMT_RETURN,
      /// \brief A DefnStmt record.
      STMT_DEFN,
      /// \brief A DefnRefExpr record.
      EXPR_DEFN_REF,
