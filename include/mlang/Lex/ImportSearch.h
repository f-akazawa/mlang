//===--- ImportSearch.h - Resolve Header File Locations ---------*- C++ -*-===//
//
...
#ifndef MLANG_LEX_IMPORT_SEARCH_H_
#define MLANG_LEX_IMPORT_SEARCH_H_
...
/// ImportedFileInfo - The preprocessor keeps track of this information for each
...
class ExternalImportedFileInfoSource {
public:
  virtual ~ExternalImportedFileInfoSource();

  /// \brief Retrieve the imported file information for the given file entry.
  ///
  /// \returns imported file information for the given file entry, with the
  /// \c External bit set. If the file entry is not known, return a
  /// default-constructed \c ImportedFileInfo.
  virtual ImportedFileInfo GetImportedFileInfo(const FileEntry *FE) = 0;
};

/// ImportSearch - This class encapsulates the information needed to find the
...
class ImportSearch {
  ...
  ///
  std::vector<ImportedFileInfo> FileInfo;
  ...
  ExternalImportedFileInfoSource *ExternalSource;
  ...
  // ImportSearch doesn't support default or copy construction.
  explicit ImportSearch();
  explicit ImportSearch(const ImportSearch&);
  void operator=(const ImportSearch&);

public:
  ImportSearch(FileManager &FM);
  ~ImportSearch();
  ...
  void SetExternalSource(ExternalImportedFileInfoSource *ES) {
    ExternalSource = ES;
  }
  ...
  void MarkFileIncludeOnce(const FileEntry *File) {
    ImportedFileInfo &FI = getFileInfo(File);
    FI.isImport = true;
  ...
  typedef std::vector<ImportedFileInfo>::const_iterator header_file_iterator;
  ...
  // Used by ASTReader.
  void setImportedFileInfoForUID(ImportedFileInfo HFI, unsigned UID);
...
private:
  /// getFileInfo - Return the ImportedFileInfo structure for the specified
  /// FileEntry.
  ImportedFileInfo &getFileInfo(const FileEntry *FE);
};

} // end namespace mlang

#endif /* IMPORTSEARCH_H_ */
