//=- MlangDiagnosticsEmitter.cpp - Generate Mlang diagnostics tables -*- C++ -*-
//
...
static void groupDiagnostics(const std::vector<Record*> &Diags,
  ...
  // groups (these are warnings that GCC supports that mlang never produces).
  for (unsigned i = 0, e = DiagGroups.size(); i != e; ++i) {
...
void MlangDiagsDefsEmitter::run(raw_ostream &OS) {
  // Write the #if guard
  if (!Component.empty()) {
...
void MlangDiagGroupsEmitter::run(raw_ostream &OS) {
...
void MlangDiagsIndexNameEmitter::run(raw_ostream &OS) {
  const std::vector<Record*> &Diags =
    Records.getAllDerivedDefinitions("Diagnostic");
