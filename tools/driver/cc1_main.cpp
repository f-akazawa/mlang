...
//===----------------------------------------------------------------------===//
//
// This is the entry point to the mlang -cc1 functionality, which implements the
...
static int cc1_test(Diagnostic &Diags,
  ...
  if (MissingArgCount)
    Diags.Report(mlang::diag::err_drv_missing_argument)
...
int cc1_main(const char **ArgBegin, const char **ArgEnd,
  ...
  if (Mlang->getImportSearchOpts().ResourceDir.empty())
    Mlang->getImportSearchOpts().ResourceDir =
      CompilerInvocation::GetResourcesPath(Argv0, MainAddr);

  // Create the actual diagnostics engine.
  Mlang->createDiagnostics(ArgEnd - ArgBegin, const_cast<char**>(ArgBegin));
  if (!Mlang->hasDiagnostics())
    return 1;

  // Set an error handler, so that any LLVM backend diagnostics go through our
  // error handler.
  llvm::install_fatal_error_handler(LLVMErrorHandler,
                                  static_cast<void*>(&Mlang->getDiagnostics()));

  DiagsBuffer->FlushDiagnostics(Mlang->getDiagnostics());

  // Execute the frontend actions.
  bool Success = ExecuteCompilerInvocation(Mlang.get());
  ...
  if (Mlang->getFrontendOpts().DisableFree) {
    if (llvm::AreStatisticsEnabled() || Mlang->getFrontendOpts().ShowStats)
