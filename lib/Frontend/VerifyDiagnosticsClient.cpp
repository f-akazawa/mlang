...
static unsigned PrintProblem(Diagnostic &Diags, SourceManager *SourceMgr,
  ...
  for (const_diag_iterator I = diag_begin, E = diag_end; I != E; ++I) {
    if (I->first.isInvalid() || !SourceMgr)
      ...
      OS << "\n  Line " << SourceMgr->getSpellingLineNumber(I->first);
    OS << ": " << I->second;
  }
...
static unsigned PrintProblem(Diagnostic &Diags, SourceManager *SourceMgr,
  ...
  for (DirectiveList::iterator I = DL.begin(), E = DL.end(); I != E; ++I) {
    ...
    if (D.Location.isInvalid() || !SourceMgr)
      ...
      OS << "\n  Line " << SourceMgr->getSpellingLineNumber(D.Location);
    OS << ": " << D.Text;
  }
...
static unsigned CheckLists(Diagnostic &Diags, SourceManager &SourceMgr,
  ...
  for (DirectiveList::iterator I = Left.begin(), E = Left.end(); I != E; ++I) {
    Directive& D = **I;
    unsigned LineNo1 = SourceMgr.getSpellingLineNumber(D.Location);

    for (unsigned i = 0; i < D.Count; ++i) {
      DiagList::iterator II, IE;
      for (II = Right.begin(), IE = Right.end(); II != IE; ++II) {
        unsigned LineNo2 = SourceMgr.getSpellingLineNumber(II->first);
        if (LineNo1 != LineNo2)
          continue;
