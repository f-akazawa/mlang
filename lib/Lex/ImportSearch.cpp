//===--- ImportSearch.cpp - XXXXXXX for Mlang  ---------------------*- C++ -*-===//
//
...
ImportedFileInfo::getControllingMacro(ExternalIdentifierLookup *External) {
  if (ControllingMacro)
...
ExternalImportedFileInfoSource::~ExternalImportedFileInfoSource() {}

ImportSearch::ImportSearch(FileManager &FM)
...
const HeaderMap *ImportSearch::CreateHeaderMap(const FileEntry *FE) {
...
const FileEntry *DirectoryLookup::DoFrameworkLookup(
  ...
  // ImportSearch.  Possible answer are yes/no and unknown.
...
/// is needed.
const FileEntry *ImportSearch::LookupFile(
    llvm::StringRef Filename,
    bool isAngled,
...
/// FileEntry.
ImportedFileInfo &ImportSearch::getFileInfo(const FileEntry *FE) {
  if (FE->getUID() >= FileInfo.size())
    FileInfo.resize(FE->getUID()+1);

  ImportedFileInfo &HFI = FileInfo[FE->getUID()];
  if (ExternalSource && !HFI.Resolved) {
    HFI = ExternalSource->GetImportedFileInfo(FE);
    HFI.Resolved = true;
  }
  return HFI;
}

bool ImportSearch::isFileMultipleIncludeGuarded(const FileEntry *File) {
  // Check if we've ever seen this file as a header.
  if (File->getUID() >= FileInfo.size())
    return false;

  // Resolve header file info from the external source, if needed.
  ImportedFileInfo &HFI = FileInfo[File->getUID()];
  if (ExternalSource && !HFI.Resolved) {
    HFI = ExternalSource->GetImportedFileInfo(File);
    HFI.Resolved = true;
  }
...
void ImportSearch::setImportedFileInfoForUID(ImportedFileInfo HFI, unsigned UID) {
...
bool ImportSearch::ShouldEnterIncludeFile(const FileEntry *File, bool isImport){
  ++NumIncluded; // Count # of attempted #includes.

  // Get information about this file.
  ImportedFileInfo &FileInfo = getFileInfo(File);
