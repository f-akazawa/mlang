...
namespace {
  ...
  public:
    ...
    virtual void HandleTopLevelDefn(DefnGroupRef D) {
      PrettyStackTraceDefn CrashInfo(*D.begin(), SourceLocation(),
    ...
    virtual void HandleTagDeclDefinition(TypeDefn *D) {
      PrettyStackTraceDefn CrashInfo((Defn *)D, SourceLocation(),
                                     Context->getSourceManager(),
                                     "LLVM IR generation of declaration");
...
static FullSourceLoc ConvertBackendLocation(const llvm::SMDiagnostic &D,
                                            SourceManager &CSM) {
  // Get both the mlang and llvm source managers.  The location is relative to
  // a memory buffer that the LLVM Source Manager is handling, we need to add
  // a copy to the Mlang source manager.
  const llvm::SourceMgr &LSM = *D.getSourceMgr();

  // We need to copy the underlying LLVM memory buffer because llvm::SourceMgr
  // already owns its one and mlang::SourceManager wants to own its one.
  const MemoryBuffer *LBuf =
  LSM.getMemoryBuffer(LSM.FindBufferContainingLoc(D.getLoc()));

  // Create the copy and transfer ownership to mlang::SourceManager.
...
void BackendConsumer::InlineAsmDiagHandler2(const llvm::SMDiagnostic &D,
  ...
  // we re-format the SMDiagnostic in terms of a mlang diagnostic.
  ...
  // If this problem has mlang-level source location information, report the
