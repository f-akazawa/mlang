...
#ifndef MLANG_CODEGEN_CGBLOCKS_H
#define MLANG_CODEGEN_CGBLOCKS_H
...
public:
  ...
  llvm::DenseMap<const VarDefn*, Capture> Captures;
  ...
  const Capture &getCapture(const VarDefn *var) const {
    llvm::DenseMap<const VarDefn*, Capture>::const_iterator
      it = Captures.find(var);
    assert(it != Captures.end() && "no entry for variable!");
    return it->second;
  }

  const ScriptDefn *getBlockDecl() const { return Block->getContainerDefn(); }
  const BlockCmd *getBlockCmd() const { return Block; }

  CGBlockInfo(const BlockCmd *blockCmd, const char *Name);
};
