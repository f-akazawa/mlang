...
//===----------------------------------------------------------------------===//
//
// TODO: This should index all interesting directories with direct calls.
//  getdirentries ?
...
class FileManager::UniqueDirContainer {
  /// UniqueRealDirs - Cache from full path to existing directories/files.
  ///
  llvm::StringMap<DirectoryEntry> UniqueRealDirs;

public:
  DirectoryEntry &getDirectory(const char *Name, struct stat &StatBuf) {
    std::string FullPath(GetFullPath(Name));
    return UniqueRealDirs.GetOrCreateValue(FullPath).getValue();
  }

  size_t size() const { return UniqueRealDirs.size(); }
};

class FileManager::UniqueFileContainer {
  /// UniqueRealFiles - Cache from full path to existing directories/files.
  ///
  llvm::StringMap<FileEntry, llvm::BumpPtrAllocator> UniqueRealFiles;
...
public:
  FileEntry &getFile(const char *Name, struct stat &StatBuf) {
    ...
    return UniqueRealFiles.GetOrCreateValue(FullPath).getValue();
  }

  size_t size() const { return UniqueRealFiles.size(); }
};
...
class FileManager::UniqueDirContainer {
  /// UniqueRealDirs - Cache from ID's to existing directories/files.
...
class FileManager::UniqueFileContainer {
  /// UniqueRealFiles - Cache from ID's to existing directories/files.
...
FileManager::FileManager(const FileSystemOptions &FSO)
  : FileSystemOpts(FSO),
    ...
    SeenDirEntries(64), SeenFileEntries(64),
    NextFileUID(0) {
...
const DirectoryEntry *FileManager::getDirectory(llvm::StringRef Filename) {
  ...
  llvm::StringMapEntry<DirectoryEntry *> &NamedDirEnt =
    SeenDirEntries.GetOrCreateValue(Filename);
...
void FileManager::PrintStats() const {
  llvm::errs() << "\n*** File Manager Stats:\n";
  llvm::errs() << UniqueRealFiles.size() << " files found, "
               << UniqueRealDirs.size() << " dirs found.\n";
