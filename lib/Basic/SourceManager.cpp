...
const llvm::MemoryBuffer *ContentCache::getBuffer(Diagnostic &Diag,
		const SourceManager &SM, SourceLocation Loc, bool *Invalid) const {
	...
	Buffer.setPointer(SM.getFileManager().getBufferForFile(
			ContentsEntry,&ErrorStr));
	...
	if (!Buffer.getPointer()) {
		...
		if (Diag.isDiagnosticInFlight())
			...
			Diag.Report(Loc, diag::err_cannot_open_file) << ContentsEntry->getName()
					<< ErrorStr;
	...
	const char
			*BOM =
					llvm::StringSwitch<const char *>(BufStr) .StartsWith(
							"\xEF\xBB\xBF", "UTF-8") .StartsWith("\xFE\xFF",
							"UTF-16 (BE)") .StartsWith("\xFF\xFE",
							"UTF-16 (LE)") .StartsWith("\x00\x00\xFE\xFF",
							"UTF-32 (BE)") .StartsWith("\xFF\xFE\x00\x00",
							"UTF-32 (LE)") .StartsWith("\x2B\x2F\x76", "UTF-7") .StartsWith(
							"\xF7\x64\x4C", "UTF-1") .StartsWith(
							"\xDD\x73\x66\x73", "UTF-EBCDIC") .StartsWith(
							"\x0E\xFE\xFF", "SDSU") .StartsWith("\xFB\xEE\x28",
							"BOCU-1") .StartsWith("\x84\x31\x95\x33",
							"GB-18030") .Default(0);

	if (BOM) {
		Diag.Report(Loc, diag::err_unsupported_bom) << BOM <<
				ContentsEntry->getName();
		Buffer.setInt(Buffer.getInt() | InvalidFlag);
	}
...
SourceManager::SourceManager(Diagnostic &Diag, FileManager &FileMgr)
  ...
  //Diagnostic class implementation is incomplete.
...
SourceLocation SourceManager::getLocation(const FileEntry *SourceFile,
  ...
  // Find the first file ID that corresponds to the given file.
		...
		if (MainSLoc.isFile()) {
			...
			if (!MainContentCache) {
				...
				SourceFileName = llvm::sys::path::filename(
						SourceFile->getName());
				if (*SourceFileName == llvm::sys::path::filename(
						MainFile->getName())) {
	...
	if (FirstFID.isInvalid()) {
		...
		for (unsigned I = 0, N = sloc_entry_size(); I != N; ++I) {
			...
			if (SLoc.isFile() && SLoc.getFile().getContentCache()
					&& SLoc.getFile().getContentCache()->OrigEntry
							== SourceFile) {
	...
	// parsed the file.
	if (FirstFID.isInvalid() && (SourceFileName || (SourceFileName
			= llvm::sys::path::filename(SourceFile->getName())))
			&& (SourceFileInode || (SourceFileInode = getActualFileInode(
					SourceFile)))) {
		bool Invalid = false;
		for (unsigned I = 0, N = sloc_entry_size(); I != N; ++I) {
			const SLocEntry &SLoc = getSLocEntry(I, &Invalid);
			...
			if (SLoc.isFile()) {
				...
				const FileEntry *Entry =
						FileContentCache ? FileContentCache->OrigEntry : 0;
				if (Entry && *SourceFileName == llvm::sys::path::filename(
						Entry->getName())) {
	...
	const char *Buf = Content->getBuffer(Diag, *this)->getBufferStart()
			+ FilePos;
	...
	return getLocForStartOfFile(FirstFID).getFileLocWithOffset(FilePos + Col
			- 1);
}
