...
#include "mlang/Driver/HostInfo.h"
#include "mlang/Driver/Option.h"
#include "mlang/Driver/Options.h"
#include "mlang/Driver/ToolChain.h"
#include "mlang/Driver/Util.h"
...
using namespace mlang::driver::tools;
...
static void CheckPreprocessingOptions(const Driver &D, const ArgList &Args) {
  if (Arg *A = Args.getLastArg(options::OPT_C, options::OPT_CC))
    if (!Args.hasArg(options::OPT_E) && !D.CCCIsCPP)
      D.Diag(mlang::diag::err_drv_argument_only_allowed_with)
...
static void CheckCodeGenerationOptions(const Driver &D, const ArgList &Args) {
  ...
  if (Args.hasArg(options::OPT_static))
    if (const Arg *A = Args.getLastArg(options::OPT_dynamic,
      ...
      D.Diag(mlang::diag::err_drv_argument_not_allowed_with)
  ...
  for (InputInfoList::const_iterator
    ...
    if (!TC.HasNativeLLVMSupport()) {
      ...
      if (II.getType() == types::TY_LLVM_IR ||
        ...
        D.Diag(mlang::diag::err_drv_no_linker_llvm_support)
          << TC.getTripleString();
    }
...
void Clang::AddPreprocessingOptions(const Driver &D,
    ...
    if (A->getOption().matches(options::OPT_include)) {
      ...
      if (FoundPCH || FoundPTH) {
        if (IsFirstImplicitInclude) {
          ...
          D.Diag(mlang::diag::warn_drv_pch_not_first_include)
  ...
  if (Arg *A = Args.getLastArg(options::OPT_I_))
    D.Diag(mlang::diag::err_drv_I_dash_not_supported) << A->getAsString(Args);
...
void Clang::AddARMTargetArgs(const ArgList &Args,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_msoft_float,
    ...
    if (A->getOption().matches(options::OPT_msoft_float))
      ...
      if (FloatABI != "soft" && FloatABI != "softfp" && FloatABI != "hard") {
        D.Diag(mlang::diag::err_drv_invalid_mfloat_abi)
          << A->getAsString(Args);
        FloatABI = "soft";
      }
    }
  ...
  if (FloatABI.empty()) {
    ...
    default:
      ...
      default:
        ...
        FloatABI = "soft";
        D.Diag(mlang::diag::warn_drv_assuming_mfloat_abi_is) << "soft";
        break;
      }
    }
  }
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_mfpu_EQ)) {
    ...
    if (FPU == "fpa" || FPU == "fpe2" || FPU == "fpe3" || FPU == "maverick") {
      ...
      D.Diag(mlang::diag::err_drv_clang_unsupported) << A->getAsString(Args);
  }
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (isa<AnalyzeJobAction>(JA)) {
    ...
    for (arg_iterator it = Args.filtered_begin(options::OPT_Wa_COMMA,
      ...
      for (unsigned i = 0, e = A->getNumValues(); i != e; ++i) {
        ...
        if (Value == "-force_cpusubtype_ALL") {
          ...
          D.Diag(mlang::diag::err_drv_unsupported_option_argument)
    ...
    assert(isa<CompileJobAction>(JA) && "Invalid action for mlang tool.");
  ...
  if (!Args.hasArg(options::OPT_fallow_unsupported)) {
    ...
    if ((Unsupported = Args.getLastArg(options::OPT_MG)) ||
      ...
      D.Diag(mlang::diag::err_drv_clang_unsupported)
    ...
    if (types::isCXX(InputType) &&
      ...
      if ((Unsupported = Args.getLastArg(options::OPT_fapple_kext)))
        D.Diag(mlang::diag::err_drv_clang_unsupported_opt_cxx_darwin_i386)
          << Unsupported->getOption().getName();
    }
  }
  ...
  // Special case debug options to only pass -g to mlang. This is
  // wrong.
  ...
  // Manually translate -O to -O2 and -O4 to -O3; let mlang reject
  // others.
  if (Arg *A = Args.getLastArg(options::OPT_O_Group)) {
  ...
  if (Arg *Std = Args.getLastArg(options::OPT_std_EQ, options::OPT_ansi)) {
    ...
    // splitting it between the driver and mlang -cc1.
    if (!types::isCXX(InputType))
...
//        << GCArg->getAsString(Args);
  ...
  // -fsigned-bitfields is default, and mlang doesn't yet support
  // -funsigned-bitfields.
  if (!Args.hasFlag(options::OPT_fsigned_bitfields,
                    options::OPT_funsigned_bitfields))
    D.Diag(mlang::diag::warn_drv_clang_unsupported)
      << Args.getLastArg(options::OPT_funsigned_bitfields)->getAsString(Args);

  // -fsigned-bitfields is default, and mlang doesn't support -fno-for-scope.
  ...
  if (Arg *A = Args.getLastArg(options::OPT_funit_at_a_time,
    ...
    if (A->getOption().matches(options::OPT_fno_unit_at_a_time))
      D.Diag(mlang::diag::warn_drv_clang_unsupported) << A->getAsString(Args);
  }

  // Default to -fno-builtin-str{cat,cpy} on Darwin for ARM.
  //
  // FIXME: This is disabled until mlang -cc1 supports -fno-builtin-foo. PR4941.
#if 0
  if (getToolChain().getTriple().getOS() == llvm::Triple::Darwin &&
  ...
  // Claim some arguments which mlang supports automatically.
  ...
  // Claim some arguments which mlang doesn't support, but we don't
  // care to warn the user about.
  Args.ClaimAllArgs(options::OPT_clang_ignored_f_Group);
  Args.ClaimAllArgs(options::OPT_clang_ignored_m_Group);

  // Disable warnings for mlang -E -use-gold-plugin -emit-llvm foo.c
...
void ClangAs::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // Don't warn about "mlang -w -c foo.s"
  Args.ClaimAllArgs(options::OPT_w);
  // and "mlang -emit-llvm -c foo.s"
  Args.ClaimAllArgs(options::OPT_emit_llvm);
  // and "mlang -use-gold-plugin -c foo.s"
  Args.ClaimAllArgs(options::OPT_use_gold_plugin);
...
void gcc::Common::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  for (InputInfoList::const_iterator
    ...
    if (II.getType() == types::TY_LLVM_IR || II.getType() == types::TY_LTO_IR ||
      ...
      D.Diag(mlang::diag::err_drv_no_ast_support)
        << getToolChain().getTripleString();
...
void gcc::Compile::RenderExtraToolArgs(const JobAction &JA,
  ...
  if (JA.getType() == types::TY_LLVM_IR || JA.getType() == types::TY_LTO_IR ||
    ...
    if (JA.getType() != types::TY_PP_Asm)
      D.Diag(mlang::diag::err_drv_invalid_gcc_output_type)
...
void darwin::Link::AddLinkArgs(Compilation &C,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mlinker_version_EQ)) {
    ...
    if (!Driver::GetReleaseVersion(A->getValue(Args), Version[0],
        ...
        HadExtra)
      D.Diag(mlang::diag::err_drv_invalid_version_number)
        << A->getAsString(Args);
  }
...
void linuxtools::Link::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // Silence warning for "mlang -g foo.o -o foo"
  Args.ClaimAllArgs(options::OPT_g_Group);
  // and "mlang -emit-llvm foo.o -o foo"
  Args.ClaimAllArgs(options::OPT_emit_llvm);
  // and for "mlang -g foo.o -o foo". Other warning options are already
