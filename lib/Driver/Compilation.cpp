  ...
  for (ArgStringList::const_iterator
    ...
    if (P.eraseFromDisk(false, &Error)) {
      ...
      if (::stat(P.c_str(), &buf) == 0 ? (buf.st_mode & S_IFMT) == S_IFREG :
                                         (errno != ENOENT)) {
        if (IssueErrors)
          getDriver().Diag(mlang::diag::err_drv_unable_to_remove_file)
            << Error;
        Success = false;
      }
    }
...
int Compilation::ExecuteCommand(const Command &C,
  ...
  if (getDriver().CCCEcho || getDriver().CCPrintOptions ||
    ...
    if (getDriver().CCPrintOptions && getDriver().CCPrintOptionsFilename) {
      ...
      if (!Error.empty()) {
        getDriver().Diag(mlang::diag::err_drv_cc_print_options_failure)
          << Error;
        FailingCommand = &C;
        delete OS;
        return 1;
  ...
  if (!Error.empty()) {
    ...
    getDriver().Diag(mlang::diag::err_drv_command_failure) << Error;
  }
