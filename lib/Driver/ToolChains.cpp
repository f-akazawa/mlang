...
using namespace mlang::driver::toolchains;
    ...
    ARCRuntimeForSimulator(ARCSimulator_None)
{
  ...
  if (!Driver::GetReleaseVersion(&OSName.c_str()[6],
    ...
    getDriver().Diag(mlang::diag::err_drv_invalid_darwin_version) << OSName;
...
void DarwinClang::AddLinkRuntimeLibArgs(const ArgList &Args,
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_static_libgcc)) {
    getDriver().Diag(mlang::diag::err_drv_unsupported_opt)
      << A->getAsString(Args);
    return;
  }
...
void Darwin::AddDeploymentTarget(DerivedArgList &Args) const {
  ...
  if (OSXVersion && (iOSVersion || iOSSimVersion)) {
    getDriver().Diag(mlang::diag::err_drv_argument_not_allowed_with)
    ...
    if (iOSSimTarget && (OSXTarget || iOSTarget)) {
      getDriver().Diag(mlang::diag::err_drv_conflicting_deployment_targets)
  ...
  if (iOSSimVersion && (getTriple().getArch() != llvm::Triple::x86 &&
    ...
    getDriver().Diag(mlang::diag::err_drv_invalid_arch_for_deployment_target)
  ...
  if (OSXVersion) {
    ...
    if (!Driver::GetReleaseVersion(OSXVersion->getValue(Args), Major, Minor,
        ...
        Major != 10 || Minor >= 100 || Micro >= 100)
      getDriver().Diag(mlang::diag::err_drv_invalid_version_number)
        << OSXVersion->getAsString(Args);
  } else {
  ...
  for (ArgList::const_iterator it = Args.begin(),
    ...
    if (A->getOption().matches(options::OPT_Xarch__)) {
      ...
      if (!XarchArg || Index > Prev + 1) {
        getDriver().Diag(mlang::diag::err_drv_invalid_Xarch_argument_with_args)
          << A->getAsString(Args);
        continue;
      } else if (XarchArg->getOption().isDriverOption()) {
        getDriver().Diag(mlang::diag::err_drv_invalid_Xarch_argument_isdriver)
          << A->getAsString(Args);
        continue;
      }
...
bool Darwin::IsUnwindTablesDefault() const {
  ...
  // definition, possibly even reusing the one in mlang.
