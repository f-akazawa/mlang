...
#ifdef HAVE_MLANG_CONFIG_H
# include "mlang/Config/config.h"
#endif
...
Driver::Driver(llvm::StringRef ClangExecutable,
  ...
  : Opts(createDriverOptTable()), Diags(Diags),
    ...
    DriverTitle("mlang \"gcc-compatible\" driver"),
    Host(0),
  ...
  if (IsProduction) {
    // In a "production" build, only use mlang on architectures we expect to
    // work, and don't use mlang C++.
    //
    // During development its more convenient to always have the driver use
    // mlang, but we don't want users to be confused when things don't work, or
  ...
  if (ClangResourceDir != "")
    ...
    llvm::sys::path::append(P, "..", "lib", "mlang", "CLANG_VERSION_STRING");
  ResourceDir = P.str();
}
...
InputArgList *Driver::ParseArgStrings(llvm::ArrayRef<const char *> ArgList) {
  ...
  if (MissingArgCount)
    Diag(mlang::diag::err_drv_missing_argument)
  ...
  for (ArgList::const_iterator it = Args->begin(), ie = Args->end();
    ...
    if (A->getOption().isUnsupported()) {
      Diag(mlang::diag::err_drv_unsupported_opt) << A->getAsString(*Args);
      continue;
    }
  }
    ...
    while (!Cur.empty()) {
      ...
      if (!Split.first.empty()) {
        ...
        if (Arch == llvm::Triple::UnknownArch)
          Diag(mlang::diag::err_drv_invalid_arch_name) << Split.first;
...
int Driver::ExecuteCompilation(const Compilation &C) const {
  ...
  if (!FailingCommand->getCreator().hasGoodDiagnostics() || Res != 1) {
    ...
    if (Res < 0)
      Diag(mlang::diag::err_drv_command_signalled)
        << FailingTool.getShortName() << -Res;
    else
      Diag(mlang::diag::err_drv_command_failed)
        << FailingTool.getShortName() << Res;
  }
...
void Driver::BuildUniversalActions(const ToolChain &TC,
  ...
  for (ArgList::const_iterator it = Args.begin(), ie = Args.end();
    ...
    if (A->getOption().matches(options::OPT_arch)) {
      ...
      if (Arch == llvm::Triple::UnknownArch) {
        Diag(mlang::diag::err_drv_invalid_arch_name)
          << A->getAsString(Args);
        continue;
      }
  ...
  if (Archs.size() > 1) {
    ...
    if (const Arg *A = Args.getLastArg(options::OPT_save_temps))
      Diag(mlang::diag::err_drv_invalid_opt_with_multiple_archs)
        << A->getAsString(Args);
  }
  ...
  for (unsigned i = 0, e = SingleActions.size(); i != e; ++i) {
    ...
    if (Archs.size() > 1 && !types::canLipoType(Act->getType()))
      Diag(mlang::diag::err_drv_invalid_output_with_multiple_archs)
...
void Driver::BuildActions(const ToolChain &TC, const DerivedArgList &Args,
    ...
    if (isa<InputOption>(A->getOption())) {
      ...
      if (InputType == types::TY_Nothing) {
        ...
        if (memcmp(Value, "-", 2) == 0) {
          ...
          if (!Args.hasArgNoClaim(options::OPT_E) && !CCCIsCPP)
            Diag(mlang::diag::err_drv_unknown_stdin_type);
          Ty = types::TY_C;
        } else {
          ...
          // If the driver is invoked as C++ compiler (like mlang++ or c++) it
          ...
          if (CCCIsCXX) {
            ...
            if (Ty != OldTy)
              Diag(mlang::diag::warn_drv_treating_input_as_cxx)
      ...
      if (CheckInputsExist && memcmp(Value, "-", 2) != 0) {
        ...
        if (/*error_code ec =*/llvm::sys::fs::exists(Value, exists) || !exists)
          Diag(mlang::diag::err_drv_no_such_file) << Path.str();
        else
      ...
      if (!InputType) {
        Diag(mlang::diag::err_drv_unknown_language) << A->getValue(Args);
  ...
  if (!SuppressMissingInputWarning && Inputs.empty()) {
    Diag(mlang::diag::err_drv_no_input_files);
    return;
  }
  ...
  if (Arg *A = Args.getLastArg(options::OPT_Z_Joined))
    Diag(mlang::diag::err_drv_use_of_Z_option) << A->getAsString(Args);
  ...
  for (unsigned i = 0, e = Inputs.size(); i != e; ++i) {
    ...
    if (InitialPhase > FinalPhase) {
      ...
      if (InitialPhase == phases::Compile && FinalPhase == phases::Preprocess &&
        ...
        Diag(mlang::diag::warn_drv_preprocessed_input_file_unused)
          << InputArg->getAsString(Args)
          << FinalPhaseArg->getOption().getName();
      else
        Diag(mlang::diag::warn_drv_input_file_unused)
          << InputArg->getAsString(Args)
...
void Driver::BuildJobs(Compilation &C) const {
  ...
  if (FinalOutput) {
    ...
    if (NumOutputs > 1) {
      Diag(mlang::diag::err_drv_output_argument_with_multiple_files);
      FinalOutput = 0;
    }
  }
  ...
  for (ArgList::const_iterator it = C.getArgs().begin(), ie = C.getArgs().end();
    ...
    if (!A->isClaimed()) {
      ...
      Diag(mlang::diag::warn_drv_unused_argument)
...
bool Driver::ShouldUseClangCompiler(const Compilation &C, const JobAction &JA,
  ...
  // Check if user requested no mlang, or mlang doesn't understand this type (we
  // only handle single inputs for now).
  if (!CCCUseClang || JA.size() != 1 ||
      !types::isAcceptedByClang((*JA.begin())->getType()))
    return false;

  // Otherwise make sure this is an action mlang understands.
  if (isa<PreprocessJobAction>(JA)) {
    if (!CCCUseClangCPP) {
      Diag(mlang::diag::warn_drv_not_using_clang_cpp);
      return false;
    }
  ...
  if (!CCCUseClangCXX && types::isCXX((*JA.begin())->getType())) {
    Diag(mlang::diag::warn_drv_not_using_clang_cxx);
    return false;
  }

  // Always use mlang for precompiling, AST generation, and rewriting,
  // regardless of archs.
  if (isa<PrecompileJobAction>(JA) ||
      types::isOnlyAcceptedByClang(JA.getType()))
    return true;

  // Finally, don't use mlang if this isn't one of the user specified archs to
  // build.
  if (!CCCClangArchs.empty() && !CCCClangArchs.count(Triple.getArch())) {
    Diag(mlang::diag::warn_drv_not_using_clang_arch) << Triple.getArchName();
    return false;
  }
